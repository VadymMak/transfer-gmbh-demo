...
import { ROUTE_PAGES, getRoutePage, localizedRouteName } from '@/lib/route-pages';
import { routeStrings, formatDuration } from '@/lib/route-pages-i18n';
import styles from './route.module.css';

export const revalidate = 60;

type RouteMeta = { nameI18n?: Record<string, string> };

async function getRouteMap(storeId: string) {
  const rows = await db.service.findMany({
    where: { storeId, active: true, category: 'route' },
    select: { slug: true, price: true, metadata: true },
  });
  const map = new Map<string, { price: number; meta: RouteMeta }>();
  for (const r of rows) {
    if (r.slug) map.set(r.slug, { price: r.price, meta: (r.metadata as RouteMeta) ?? {} });
  }
  return map;
}
...
export async function generateMetadata({
  ...
  const dbRow = map.get(def.slug);
  const price = dbRow?.price ?? def.price;
  const name = localizedRouteName(def, dbRow?.meta.nameI18n, locale);
  const dest = destOf(name);
  const brand = config.name || 'Euroschnell';

  const title = `${name} — ${s.fixedPrice} ${price} € | ${brand}`;
  ...
  return {
    title,
    description,
    alternates: { canonical: `${baseUrl}/${locale}/transfer/${slug}`, languages },
    openGraph: { title, description, type: 'website', url: `${baseUrl}/${locale}/transfer/${slug}` },
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.facts}>
        ...
        <div className={styles.fact}>
          ...
          <span className={styles.factValue}>{price} €</span>
        </div>
      </div>
      ...
      <p className={styles.intro}>{s.intro({ dest, dist: def.distanceKm, dur, price })}</p>
